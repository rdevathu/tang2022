...
def select_8_independent_leads(
    ecg_data: np.ndarray, lead_names: List[str], verbose: bool = False
) -> np.ndarray:
    """
    ...
    Args:
        ...
        verbose: If True, print lead selection information
    ...
    if not lead_names or len(lead_names) != ecg_data.shape[1]:
        if ecg_data.shape[1] == 12:
            ...
            raise ValueError(
                f"Cannot infer lead names for ECG with {ecg_data.shape[1]} channels"
            )
    ...
    if missing_leads:
        raise ValueError(
            f"Missing required leads for Tang et al. 2022: {missing_leads}. "
    ...
    if verbose:
        ...
        print(
            f"From original {len(normalized_leads)} leads: {normalized_leads}"
        )
...
if __name__ == "__main__":
    ...
    # Select 8 independent leads (with verbose output for testing)
    selected_ecg = select_8_independent_leads(
        dummy_ecg, lead_names, verbose=True
    )

    print(f"Selected ECG shape: {selected_ecg.shape}")
    print(f"Expected shape: ({n_samples}, 8)")

    assert selected_ecg.shape == (n_samples, 8), (
        f"Wrong shape: {selected_ecg.shape}"
    )

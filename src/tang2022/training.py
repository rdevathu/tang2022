...
from .dataset import create_dataloaders, aggregate_patient_predictions
...
class ECGTrainer:
    ...
    def evaluate_epoch(self, test_loader: DataLoader, 
        ...
        with torch.no_grad():
            for windows, labels, patient_ids in tqdm(test_loader, desc="Evaluating"):
                windows = windows.to(self.device)
                ...
                for i, patient_id in enumerate(patient_ids):
                    ...
                    patient_predictions[patient_id].append(probabilities[i].cpu().item())
    ...
    def calculate_ece(self, y_true: np.ndarray, y_prob: np.ndarray, 
                     n_bins: int = 10) -> float:
        """
...
def run_cross_validation(carto_dataset: CARTODataset,
                        n_folds: int = 10,
                        n_epochs: int = 50,
                        batch_size: int = 32,
                        learning_rate: float = 1e-3,
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, labels)):
        ...
        train_loader, test_loader = create_dataloaders(
            ...
            batch_size=batch_size, augmentation=augmentation, num_workers=0
        )
        ...
        training_results = trainer.train(
            ...
            patience=10, save_path=str(fold_save_path)
        )
        
        # Final evaluation
        final_metrics = trainer.evaluate_epoch(test_loader, return_predictions=True)
        ...
        fold_result_summary = {k: v for k, v in fold_result.items() if k != 'predictions'}
    ...
    metric_cols = ['auroc', 'sensitivity', 'specificity', 'accuracy', 'f1', 
                   'precision', 'recall', 'brier_score', 'ece']
